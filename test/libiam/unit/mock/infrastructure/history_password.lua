...
function HistoryPassword:ctor(db, account_id)
    ...
    -- TODO：待讨论infrastructure是否能使用Domain的config
    self.m_global_account_config = global_account_config.get_instance()
end

--- 插入指定用户历史密码
---@param password string
function HistoryPassword:insert(password, kdf_password)
    local max_count = self.m_global_account_config:get_history_password_count()
    if max_count == 0 then
        return
    end
...
function HistoryPassword:update(current_password, kdf_password)
    local max_count = self.m_global_account_config:get_history_password_count()
    local history_password_list = self:get()
    if #history_password_list == 0 and max_count > 0 then
    ...
    self.db:delete(self.db.HistoryPassword)
        :where(self.db.HistoryPassword.AccountId:eq(self.m_account_id),
            self.db.HistoryPassword.SequenceNumber:gt(max_count)):all()
end
...
function HistoryPassword:check(password)
    ...
    for _, row in pairs(history_password_list) do
        if not (row.Password or row.KDFPassword) then
            ...
            local same = self:check_password_same(row.KDFPassword, password, user_config.SHA512_SALT_PATTERN) or
                self:check_password_same(row.Password, password, user_config.SHA512_SALT_PATTERN)
