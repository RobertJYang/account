...
function TestIam:test_check_one_ip_lock()
    ...
    -- 默认拿到的锁定状态是false
    lu.assertIsFalse(iam_core.get_one_ip_lock_status(self.ip_lock_dir, TEST_IP_1, lock_threshold, fail_interval, unlock_time))

    -- 增加三次记录后，拿到的锁定状态为true
    for _ = 1, lock_threshold do
        lu.assertEquals(iam_core.increase_ip_fail_record(self.ip_lock_dir, TEST_IP_1), 0)
    end
    lu.assertIsTrue(iam_core.get_one_ip_lock_status(self.ip_lock_dir, TEST_IP_1, lock_threshold, fail_interval, unlock_time))
...
function TestIam:test_two_ip_lock_is_independent_of_other()
    ...
    lu.assertIsTrue(iam_core.get_one_ip_lock_status(self.ip_lock_dir, TEST_IP_1, lock_threshold, fail_interval, unlock_time))
    lu.assertIsFalse(iam_core.get_one_ip_lock_status(self.ip_lock_dir, TEST_IP_2, lock_threshold, fail_interval, unlock_time))
...
function TestIam:test_get_all_ip_lock_records()
    ...
    for k, v in pairs(suit_table) do
        for _ = 1, v.time do
            lu.assertEquals(iam_core.increase_ip_fail_record(self.ip_lock_dir, k), 0)
        end
    end

    -- 查看预期
    local count, records = iam_core.get_all_ip_lock_status(self.ip_lock_dir, lock_threshold, fail_interval, unlock_time)
...
function TestIam:test_all_ip_records_cnt()
    -- 首先增加记录到1000条，均成功
    for i = 1, 1000 do
        lu.assertEquals(iam_core.increase_ip_fail_record(self.ip_lock_dir, tostring(i)), 0)
    end

    -- 再进行尝试，无法增加
    lu.assertEquals(iam_core.increase_ip_fail_record(self.ip_lock_dir, "test"), -1)

    -- 尝试增加已有的ip的记录，可以成功
    lu.assertEquals(iam_core.increase_ip_fail_record(self.ip_lock_dir, "111"), 0)

    -- 清理锁定记录
    for i = 1, 1000 do
        lu.assertEquals(iam_core.clean_ip_fail_record(self.ip_lock_dir, tostring(i)), 0)
    end
end