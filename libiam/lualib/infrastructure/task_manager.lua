...
function TaskManager:ctor(bus)
    self.bus = bus
    -- 默认任务不生成时，没有相关的任务id
    self.ssh_import_task_id = nil
end

function TaskManager:create_ssh_import_task(account_id)
...
function TaskManager:create_weakpwddic_import_task()
    local name = 'Import File Task'
...
function TaskManager:create_weakpwddic_export_task()
...
function TaskManager:_update_task(task_id, is_success, error_msg)
    local data, log_msg
    if is_success then
...
function TaskManager:update_ssh_import_task(is_success, error_msg)
    self:_update_task(self.ssh_import_task_id, is_success, error_msg)
end

function TaskManager:update_weakpwddic_import_task(is_success, error_msg)
    self:_update_task(self.weakpwddict_import_task_id, is_success, error_msg)
end

function TaskManager:update_weakpwddic_export_task(is_success, error_msg)
