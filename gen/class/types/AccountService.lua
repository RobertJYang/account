...
function TPreviousPasswordsDisallowed.new(PreviousPasswordsDisallowed)
    return TPreviousPasswordsDisallowed_from_obj({
        PreviousPasswordsDisallowed = PreviousPasswordsDisallowed or 5
    })
end
...
TPreviousPasswordsDisallowed.struct = {
    {name = 'PreviousPasswordsDisallowed', is_array = false, struct = nil}
}

function TPreviousPasswordsDisallowed:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'PreviousPasswordsDisallowed',
                      self.PreviousPasswordsDisallowed, "uint8", false, errs,
                      need_convert)

    TPreviousPasswordsDisallowed:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self,
                                 TPreviousPasswordsDisallowed.proto_property,
                                 errs, need_convert)
    return self
end
...
function TPasswordComplexityIsLock.new(PasswordComplexityIsLock)
    return TPasswordComplexityIsLock_from_obj({
        PasswordComplexityIsLock = PasswordComplexityIsLock or false
    })
end
...
TPasswordComplexityIsLock.struct = {
    {name = 'PasswordComplexityIsLock', is_array = false, struct = nil}
}

function TPasswordComplexityIsLock:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'PasswordComplexityIsLock',
                      self.PasswordComplexityIsLock, "bool", false, errs,
                      need_convert)

    TPasswordComplexityIsLock:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TPasswordComplexityIsLock.proto_property,
                                 errs, need_convert)
    return self
end

function TPasswordComplexityIsLock:unpack(_) return
    self.PasswordComplexityIsLock end
...
TTimeSource.struct = {
    {
        name = 'TimeSource',
        is_array = false,
        struct = def_types.TimeSource.struct
    }
}
...
function TTimeSource:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'TimeSource', self.TimeSource,
                      "def_types.TimeSource", false, errs, need_convert)

    TTimeSource:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TTimeSource.proto_property, errs,
                                 need_convert)
    return self
end
...
function TUserMgmtEnable.new(UserMgmtEnable)
    return TUserMgmtEnable_from_obj({
        UserMgmtEnable = UserMgmtEnable == nil and true or UserMgmtEnable
    })
end
---@param obj MAccountService.UserMgmtEnable
function TUserMgmtEnable:init_from_obj(obj)
    self.UserMgmtEnable = obj.UserMgmtEnable == nil and true or
                              obj.UserMgmtEnable
end
...
TUserMgmtEnable.struct = {
    {name = 'UserMgmtEnable', is_array = false, struct = nil}
}

function TUserMgmtEnable:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'UserMgmtEnable', self.UserMgmtEnable, "bool",
                      false, errs, need_convert)

    TUserMgmtEnable:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TUserMgmtEnable.proto_property, errs,
                                 need_convert)
    return self
end

function TUserMgmtEnable:unpack(_) return self.UserMgmtEnable end
...
function TAccountLockoutThreshold.new(AccountLockoutThreshold)
    return TAccountLockoutThreshold_from_obj({
        AccountLockoutThreshold = AccountLockoutThreshold or 5
    })
end
...
TAccountLockoutThreshold.struct = {
    {name = 'AccountLockoutThreshold', is_array = false, struct = nil}
}

function TAccountLockoutThreshold:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'AccountLockoutThreshold',
                      self.AccountLockoutThreshold, "int32", false, errs,
                      need_convert)

    TAccountLockoutThreshold:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TAccountLockoutThreshold.proto_property,
                                 errs, need_convert)
    return self
end

function TAccountLockoutThreshold:unpack(_) return self.AccountLockoutThreshold end
...
function TAccountLockoutDuration.new(AccountLockoutDuration)
    return TAccountLockoutDuration_from_obj({
        AccountLockoutDuration = AccountLockoutDuration or 300
    })
end
...
TAccountLockoutDuration.struct = {
    {name = 'AccountLockoutDuration', is_array = false, struct = nil}
}

function TAccountLockoutDuration:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'AccountLockoutDuration',
                      self.AccountLockoutDuration, "int32", false, errs,
                      need_convert)

    TAccountLockoutDuration:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TAccountLockoutDuration.proto_property,
                                 errs, need_convert)
    return self
end

function TAccountLockoutDuration:unpack(_) return self.AccountLockoutDuration end
...
function TPasswordExpirationDays.new(PasswordExpirationDays)
    return TPasswordExpirationDays_from_obj({
        PasswordExpirationDays = PasswordExpirationDays or 4294967295
    })
end
...
TPasswordExpirationDays.struct = {
    {name = 'PasswordExpirationDays', is_array = false, struct = nil}
}

function TPasswordExpirationDays:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'PasswordExpirationDays',
                      self.PasswordExpirationDays, "uint32", false, errs,
                      need_convert)

    TPasswordExpirationDays:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TPasswordExpirationDays.proto_property,
                                 errs, need_convert)
    return self
end

function TPasswordExpirationDays:unpack(_) return self.PasswordExpirationDays end
