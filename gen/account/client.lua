...
local get_non_virtual_interface_objects =
    mdb_service.get_non_virtual_interface_objects
local foreach_non_virtual_interface_objects =
    mdb_service.foreach_non_virtual_interface_objects
...
function account_client:GetCipherSuitObjects()
    return get_non_virtual_interface_objects(self:get_bus(),
...
function account_client:GetTaskObjects()
    return get_non_virtual_interface_objects(self:get_bus(),
                                             'bmc.kepler.TaskService.Task', true)
end
...
function account_client:GetEventsObjects()
    return get_non_virtual_interface_objects(self:get_bus(),
                                             'bmc.kepler.Systems.Events', true)
end
...
function account_client:GetIpmiCoreObjects()
    return get_non_virtual_interface_objects(self:get_bus(),
                                             'bmc.kepler.IpmiCore', true)
end

function account_client:ForeachIpmiCoreObjects(cb)
    return foreach_non_virtual_interface_objects(self:get_bus(),
                                                 'bmc.kepler.IpmiCore', cb, true)
end
...
function account_client:OnCipherSuitPropertiesChanged(cb)
    self.signal_slots[#self.signal_slots + 1] =
        subscribe_signal.on_properties_changed(self:get_bus(), '/bmc', cb,
                                               'bmc.kepler.Managers.Security.TlsConfig.CipherSuit')
end

function account_client:OnCipherSuitInterfacesAdded(cb)
    self.signal_slots[#self.signal_slots + 1] =
        subscribe_signal.on_interfaces_added(self:get_bus(), '/bmc', cb,
                                             'bmc.kepler.Managers.Security.TlsConfig.CipherSuit')
end

function account_client:OnCipherSuitInterfacesRemoved(cb)
    self.signal_slots[#self.signal_slots + 1] =
        subscribe_signal.on_interfaces_removed(self:get_bus(), '/bmc', cb,
                                               'bmc.kepler.Managers.Security.TlsConfig.CipherSuit')
end
...
function account_client:SubscribeIpv4ChangedSignal(cb)
    local sig = match_rule.signal('ChangedSignal',
                                  'bmc.kepler.Managers.EthernetInterfaces.Ipv4')
    self.signal_slots[#self.signal_slots + 1] =
        self:get_bus():match(sig, function(msg) cb(msg:read()) end)
end
...
function account_client:SubscribeEthernetInterfacesActivePortChangedSignal(cb)
    local sig = match_rule.signal('ActivePortChangedSignal',
                                  'bmc.kepler.Managers.EthernetInterfaces')
    self.signal_slots[#self.signal_slots + 1] =
        self:get_bus():match(sig, function(msg) cb(msg:read()) end)
end
...
function account_client:SubscribeEthernetInterfacesNCSIInfoChangedSignal(cb)
    local sig = match_rule.signal('NCSIInfoChangedSignal',
                                  'bmc.kepler.Managers.EthernetInterfaces')
    self.signal_slots[#self.signal_slots + 1] =
        self:get_bus():match(sig, function(msg) cb(msg:read()) end)
end
...
function account_client:SubscribeEthernetInterfacesEthMacChangedSignal(cb)
    local sig = match_rule.signal('EthMacChangedSignal',
                                  'bmc.kepler.Managers.EthernetInterfaces')
    self.signal_slots[#self.signal_slots + 1] =
        self:get_bus():match(sig, function(msg) cb(msg:read()) end)
end

function account_client:ctor() self.signal_slots = {} end
