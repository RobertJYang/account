...
function TGenRmcp15CodeRsp:init_from_obj(obj) self.AuthCode = obj.AuthCode end
...
function TGenRmcp15CodeRsp:validate(prefix, errs, need_convert)
    ...
    validate.OptionalArray(prefix .. 'AuthCode', self.AuthCode, "uint8", false,
                           errs, need_convert)

    TGenRmcp15CodeRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TGenRmcp15CodeRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TGenRmcp15CodeRsp:unpack(_) return self.AuthCode end
...
function TGenRmcp15CodeReq.new(AuthAlgo, PayLoad, AccountId, SessionId,
                               SessionSequence)
...
TGenRmcp15CodeReq.proto_property = {
    'AuthAlgo', 'PayLoad', 'AccountId', 'SessionId', 'SessionSequence'
}
...
function TGenRmcp15CodeReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'AuthAlgo', self.AuthAlgo, "uint8", false, errs,
                      need_convert)
    ...
    validate.Optional(prefix .. 'AccountId', self.AccountId, "uint8", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'SessionId', self.SessionId, "uint32", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'SessionSequence', self.SessionSequence,
    ...
    validate.CheckUnknowProperty(self, TGenRmcp15CodeReq.proto_property, errs,
                                 need_convert)
    return self
end

function TGenRmcp15CodeReq:unpack(_)
    return self.AuthAlgo, self.PayLoad, self.AccountId, self.SessionId,
           self.SessionSequence
end
...
function TGenRmcp20CodeRsp.new(Rap2AuthCode, Sik, Rap3AuthCode)
    return TGenRmcp20CodeRsp_from_obj({
        Rap2AuthCode = Rap2AuthCode,
        Sik = Sik,
        Rap3AuthCode = Rap3AuthCode
    })
end
...
TGenRmcp20CodeRsp.struct = {
    {name = 'Rap2AuthCode', is_array = true, struct = nil},
    {name = 'Sik', is_array = true, struct = nil},
...
function TGenRmcp20CodeRsp:validate(prefix, errs, need_convert)
    ...
    validate.OptionalArray(prefix .. 'Rap2AuthCode', self.Rap2AuthCode, "uint8",
                           false, errs, need_convert)
    validate.OptionalArray(prefix .. 'Sik', self.Sik, "uint8", false, errs,
                           need_convert)
    validate.OptionalArray(prefix .. 'Rap3AuthCode', self.Rap3AuthCode, "uint8",
                           false, errs, need_convert)

    TGenRmcp20CodeRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TGenRmcp20CodeRsp.proto_property, errs,
                                 need_convert)
    return self
end
...
function TGenRmcp20CodeReq.new(AuthAlgo, UserName, ConsoleSid, ManagedSid,
                               ConsoleRandom, ManagedRandom, ManagedGuid, Role,
                               Ip)
...
TGenRmcp20CodeReq.proto_property = {
    'AuthAlgo', 'UserName', 'ConsoleSid', 'ManagedSid', 'ConsoleRandom',
...
function TGenRmcp20CodeReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'UserName', self.UserName, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'ConsoleSid', self.ConsoleSid, "uint32", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'ManagedSid', self.ManagedSid, "uint32", false,
                      errs, need_convert)
    validate.OptionalArray(prefix .. 'ConsoleRandom', self.ConsoleRandom,
                           "uint8", false, errs, need_convert)
    validate.OptionalArray(prefix .. 'ManagedRandom', self.ManagedRandom,
                           "uint8", false, errs, need_convert)
    validate.OptionalArray(prefix .. 'ManagedGuid', self.ManagedGuid, "uint8",
                           false, errs, need_convert)
    validate.Optional(prefix .. 'Role', self.Role, "uint8", false, errs,
                      need_convert)
    ...
    if self.ConsoleRandom ~= nil then
        validate.lens(prefix .. 'ConsoleRandom', self.ConsoleRandom, 16, 16,
                      errs, need_convert)
    end
    if self.ManagedRandom ~= nil then
        validate.lens(prefix .. 'ManagedRandom', self.ManagedRandom, 16, 16,
                      errs, need_convert)
    end
    ...
    validate.CheckUnknowProperty(self, TGenRmcp20CodeReq.proto_property, errs,
                                 need_convert)
    return self
end

function TGenRmcp20CodeReq:unpack(_)
    return self.AuthAlgo, self.UserName, self.ConsoleSid, self.ManagedSid,
           self.ConsoleRandom, self.ManagedRandom, self.ManagedGuid, self.Role,
           self.Ip
end
...
function TVncAuthenticateReq.new(CipherText, AuthChallenge)
    return TVncAuthenticateReq_from_obj({
...
function TVncAuthenticateReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'CipherText', self.CipherText, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'AuthChallenge', self.AuthChallenge, "string",
                      false, errs, need_convert)

    TVncAuthenticateReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TVncAuthenticateReq.proto_property, errs,
                                 need_convert)
    return self
end

function TVncAuthenticateReq:unpack(_) return self.CipherText,
                                              self.AuthChallenge end
...
local function TAccountData_from_obj(obj) return setmetatable(obj, TAccountData) end

function TAccountData.new(dict) return TAccountData_from_obj(dict) end

---@param obj LocalAccountAuthN.AccountData
function TAccountData:init_from_obj(obj) self = obj end
...
function TAccountData:unpack(_) return self end
...
TVncAuthenticateRsp.struct = {
    {
        name = 'AccountData',
        is_array = false,
        struct = LocalAccountAuthN.AccountData.struct
    }
}
...
function TVncAuthenticateRsp:validate(prefix, errs, need_convert)
    ...
    LocalAccountAuthN.AccountData.new(self.AccountData):validate(prefix, errs,
                                                                 need_convert)

    TVncAuthenticateRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TVncAuthenticateRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TVncAuthenticateRsp:unpack(_) return self.AccountData end
...
function TLocalAuthenticateRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TLocalAuthenticateRsp.proto_property,
                                 errs, need_convert)
    return self
end

function TLocalAuthenticateRsp:unpack(_) return self.AccountData end
...
local function TConfig_from_obj(obj) return setmetatable(obj, TConfig) end

function TConfig.new(dict) return TConfig_from_obj(dict) end

---@param obj LocalAccountAuthN.Config
function TConfig:init_from_obj(obj) self = obj end
...
function TConfig:unpack(_) return self end
...
function TLocalAuthenticateReq.new(UserName, Password, Config)
    return TLocalAuthenticateReq_from_obj({
        UserName = UserName,
        Password = Password,
        Config = Config
    })
...
TLocalAuthenticateReq.struct = {
    ...
    {name = 'Password', is_array = true, struct = nil},
    {
        name = 'Config',
        is_array = false,
        struct = LocalAccountAuthN.Config.struct
    }
}
...
function TLocalAuthenticateReq:validate(prefix, errs, need_convert)
    ...
    LocalAccountAuthN.Config.new(self.Config):validate(prefix, errs,
                                                       need_convert)
    ...
    validate.CheckUnknowProperty(self, TLocalAuthenticateReq.proto_property,
                                 errs, need_convert)
    return self
end
...
LocalAccountAuthN.interface = mdb.register_interface(
                                  'bmc.kepler.AccountService.LocalAccountAuthN',
                                  {}, {
        LocalAuthenticate = {
            'a{ss}saya{sb}', 'a{ss}', TLocalAuthenticateReq,
            TLocalAuthenticateRsp
        },
        VncAuthenticate = {
            'a{ss}ss', 'a{ss}', TVncAuthenticateReq, TVncAuthenticateRsp
        },
        GenRmcp20Code = {
            'a{ss}ysuuayayayys', 'ayayay', TGenRmcp20CodeReq, TGenRmcp20CodeRsp
        },
        GenRmcp15Code = {
            'a{ss}yayyuu', 'ay', TGenRmcp15CodeReq, TGenRmcp15CodeRsp
        }
    }, {})
