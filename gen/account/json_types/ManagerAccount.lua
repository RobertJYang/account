...
function TPrivileges:init_from_obj(obj) self.Privileges = obj.Privileges end
...
function TPrivileges:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TPrivileges.proto_property, errs,
                                 need_convert)
    return self
end

function TPrivileges:unpack(_) return self.Privileges end
...
function TLoginInterface:validate(prefix, errs, need_convert)
    ...
    validate.OptionalArray(prefix .. 'LoginInterface', self.LoginInterface,
                           "string", false, errs, need_convert)

    TLoginInterface:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TLoginInterface.proto_property, errs,
                                 need_convert)
    return self
end

function TLoginInterface:unpack(_) return self.LoginInterface end
...
function TFirstLoginPolicy.new(FirstLoginPolicy)
    return
        TFirstLoginPolicy_from_obj({FirstLoginPolicy = FirstLoginPolicy or 2})
end
...
TFirstLoginPolicy.struct = {
    {name = 'FirstLoginPolicy', is_array = false, struct = nil}
}

function TFirstLoginPolicy:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'FirstLoginPolicy', self.FirstLoginPolicy,
                      "uint8", false, errs, need_convert)

    TFirstLoginPolicy:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TFirstLoginPolicy.proto_property, errs,
                                 need_convert)
    return self
end

function TFirstLoginPolicy:unpack(_) return self.FirstLoginPolicy end
...
function TLastLoginInterface.new(LastLoginInterface)
    return TLastLoginInterface_from_obj({
        LastLoginInterface = LastLoginInterface
    })
end
...
TLastLoginInterface.struct = {
    {name = 'LastLoginInterface', is_array = false, struct = nil}
}

function TLastLoginInterface:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'LastLoginInterface', self.LastLoginInterface,
                      "string", true, errs, need_convert)

    TLastLoginInterface:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TLastLoginInterface.proto_property, errs,
                                 need_convert)
    return self
end

function TLastLoginInterface:unpack(_) return self.LastLoginInterface end
...
local function TLastLoginIP_from_obj(obj) return setmetatable(obj, TLastLoginIP) end
...
function TLastLoginIP:init_from_obj(obj) self.LastLoginIP = obj.LastLoginIP end
...
function TLastLoginIP:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'LastLoginIP', self.LastLoginIP, "string", true,
                      errs, need_convert)

    TLastLoginIP:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TLastLoginIP.proto_property, errs,
                                 need_convert)
    return self
end

function TLastLoginIP:unpack(_) return self.LastLoginIP end
...
function TLastLoginTime:init_from_obj(obj) self.LastLoginTime =
    obj.LastLoginTime end
...
TLastLoginTime.struct = {
    {name = 'LastLoginTime', is_array = false, struct = nil}
}

function TLastLoginTime:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'LastLoginTime', self.LastLoginTime, "uint32",
                      true, errs, need_convert)

    TLastLoginTime:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TLastLoginTime.proto_property, errs,
                                 need_convert)
    return self
end

function TLastLoginTime:unpack(_) return self.LastLoginTime end
...
function TLoginRuleIds:init_from_obj(obj) self.LoginRuleIds = obj.LoginRuleIds end
...
function TLoginRuleIds:validate(prefix, errs, need_convert)
    ...
    validate.OptionalArray(prefix .. 'LoginRuleIds', self.LoginRuleIds,
                           "string", false, errs, need_convert)

    TLoginRuleIds:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TLoginRuleIds.proto_property, errs,
                                 need_convert)
    return self
end

function TLoginRuleIds:unpack(_) return self.LoginRuleIds end
...
function TAccountType:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'AccountType', self.AccountType, "string", true,
                      errs, need_convert)
...
TSshPublicKeyHash.struct = {
    {name = 'SshPublicKeyHash', is_array = false, struct = nil}
}

function TSshPublicKeyHash:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'SshPublicKeyHash', self.SshPublicKeyHash,
                      "string", true, errs, need_convert)

    TSshPublicKeyHash:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TSshPublicKeyHash.proto_property, errs,
                                 need_convert)
    return self
end

function TSshPublicKeyHash:unpack(_) return self.SshPublicKeyHash end
...
local function TRoleId_from_obj(obj) return setmetatable(obj, TRoleId) end

function TRoleId.new(RoleId) return TRoleId_from_obj({RoleId = RoleId}) end
---@param obj ManagerAccount.RoleId
function TRoleId:init_from_obj(obj) self.RoleId = obj.RoleId end
...
function TRoleId:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'RoleId', self.RoleId, "uint8", false, errs,
                      need_convert)

    TRoleId:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TRoleId.proto_property, errs,
                                 need_convert)
    return self
end

function TRoleId:unpack(_) return self.RoleId end
...
function TPasswordExpiration.new(PasswordExpiration)
    return TPasswordExpiration_from_obj({
...
TPasswordExpiration.struct = {
    {name = 'PasswordExpiration', is_array = false, struct = nil}
}

function TPasswordExpiration:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'PasswordExpiration', self.PasswordExpiration,
                      "uint32", true, errs, need_convert)

    TPasswordExpiration:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TPasswordExpiration.proto_property, errs,
                                 need_convert)
    return self
end

function TPasswordExpiration:unpack(_) return self.PasswordExpiration end
...
function TPasswordChangeRequired.new(PasswordChangeRequired)
    return TPasswordChangeRequired_from_obj({
        PasswordChangeRequired = PasswordChangeRequired == nil and true or
            PasswordChangeRequired
    })
end
---@param obj ManagerAccount.PasswordChangeRequired
function TPasswordChangeRequired:init_from_obj(obj)
    self.PasswordChangeRequired = obj.PasswordChangeRequired == nil and true or
                                      obj.PasswordChangeRequired
end
...
TPasswordChangeRequired.struct = {
    {name = 'PasswordChangeRequired', is_array = false, struct = nil}
}

function TPasswordChangeRequired:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'PasswordChangeRequired',
                      self.PasswordChangeRequired, "bool", false, errs,
                      need_convert)

    TPasswordChangeRequired:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TPasswordChangeRequired.proto_property,
                                 errs, need_convert)
    return self
end

function TPasswordChangeRequired:unpack(_) return self.PasswordChangeRequired end
...
local function TDeletable_from_obj(obj) return setmetatable(obj, TDeletable) end

function TDeletable.new(Deletable)
    return TDeletable_from_obj({Deletable = Deletable})
end
---@param obj ManagerAccount.Deletable
function TDeletable:init_from_obj(obj) self.Deletable = obj.Deletable end
...
function TDeletable:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Deletable', self.Deletable, "bool", true, errs,
                      need_convert)

    TDeletable:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TDeletable.proto_property, errs,
                                 need_convert)
    return self
end

function TDeletable:unpack(_) return self.Deletable end
...
local function TUserName_from_obj(obj) return setmetatable(obj, TUserName) end

function TUserName.new(UserName) return
    TUserName_from_obj({UserName = UserName}) end
---@param obj ManagerAccount.UserName
function TUserName:init_from_obj(obj) self.UserName = obj.UserName end
...
function TUserName:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TUserName.proto_property, errs,
                                 need_convert)
    return self
end

function TUserName:unpack(_) return self.UserName end
...
local function TLocked_from_obj(obj) return setmetatable(obj, TLocked) end

function TLocked.new(Locked) return TLocked_from_obj({Locked = Locked}) end
---@param obj ManagerAccount.Locked
function TLocked:init_from_obj(obj) self.Locked = obj.Locked end
...
function TLocked:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Locked', self.Locked, "bool", true, errs,
                      need_convert)

    TLocked:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TLocked.proto_property, errs,
                                 need_convert)
    return self
end

function TLocked:unpack(_) return self.Locked end
...
function TCertificates:init_from_obj(obj) self.Certificates = obj.Certificates end
...
function TCertificates:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Certificates', self.Certificates, "uint16",
                      true, errs, need_convert)

    TCertificates:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TCertificates.proto_property, errs,
                                 need_convert)
    return self
end

function TCertificates:unpack(_) return self.Certificates end
...
TAccountExpiration.struct = {
    {name = 'AccountExpiration', is_array = false, struct = nil}
}

function TAccountExpiration:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'AccountExpiration', self.AccountExpiration,
                      "string", true, errs, need_convert)

    TAccountExpiration:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TAccountExpiration.proto_property, errs,
                                 need_convert)
    return self
end

function TAccountExpiration:unpack(_) return self.AccountExpiration end
...
function TSetLastLoginRsp:init_from_obj(obj) self.Result = obj.Result end
...
function TSetLastLoginRsp:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Result', self.Result, "uint8", false, errs,
                      need_convert)

    TSetLastLoginRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TSetLastLoginRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TSetLastLoginRsp:unpack(_) return self.Result end
...
function TSetLastLoginReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Ip', self.Ip, "string", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'Interface', self.Interface, "string", false,
                      errs, need_convert)

    TSetLastLoginReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TSetLastLoginReq.proto_property, errs,
                                 need_convert)
    return self
end

function TSetLastLoginReq:unpack(_) return self.Ip, self.Interface end
...
function TDeleteSSHPublicKeyRsp.new() return TDeleteSSHPublicKeyRsp_from_obj({}) end
---@param obj ManagerAccount.DeleteSSHPublicKeyRsp
function TDeleteSSHPublicKeyRsp:init_from_obj(obj) end
...
function TDeleteSSHPublicKeyRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TDeleteSSHPublicKeyRsp.proto_property,
                                 errs, need_convert)
    return self
end

function TDeleteSSHPublicKeyRsp:unpack(_) end
...
function TDeleteSSHPublicKeyReq.new() return TDeleteSSHPublicKeyReq_from_obj({}) end
---@param obj ManagerAccount.DeleteSSHPublicKeyReq
function TDeleteSSHPublicKeyReq:init_from_obj(obj) end
...
function TDeleteSSHPublicKeyReq:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TDeleteSSHPublicKeyReq.proto_property,
                                 errs, need_convert)
    return self
end

function TDeleteSSHPublicKeyReq:unpack(_) end
...
function TImportSSHPublicKeyRsp:init_from_obj(obj) self.TaskId = obj.TaskId end
...
function TImportSSHPublicKeyRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TImportSSHPublicKeyRsp.proto_property,
                                 errs, need_convert)
    return self
end

function TImportSSHPublicKeyRsp:unpack(_) return self.TaskId end
...
function TImportSSHPublicKeyReq:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TImportSSHPublicKeyReq.proto_property,
                                 errs, need_convert)
    return self
end

function TImportSSHPublicKeyReq:unpack(_) return self.Type, self.Content end
...
function TChangeSnmpPwdRsp.new() return TChangeSnmpPwdRsp_from_obj({}) end
---@param obj ManagerAccount.ChangeSnmpPwdRsp
function TChangeSnmpPwdRsp:init_from_obj(obj) end
...
function TChangeSnmpPwdRsp:unpack(_) end
...
function TChangeSnmpPwdReq:init_from_obj(obj) self.Password = obj.Password end
...
function TChangeSnmpPwdReq:validate(prefix, errs, need_convert)
    ...
    validate.OptionalArray(prefix .. 'Password', self.Password, "uint8", false,
                           errs, need_convert)
...
function TChangeSnmpPwdReq:unpack(_) return self.Password end
...
function TChangePwdRsp.new() return TChangePwdRsp_from_obj({}) end
---@param obj ManagerAccount.ChangePwdRsp
function TChangePwdRsp:init_from_obj(obj) end
...
function TChangePwdRsp:unpack(_) end
...
function TChangePwdReq:init_from_obj(obj) self.Password = obj.Password end
...
function TChangePwdReq:unpack(_) return self.Password end
...
local function TDeleteRsp_from_obj(obj) return setmetatable(obj, TDeleteRsp) end

function TDeleteRsp.new() return TDeleteRsp_from_obj({}) end
---@param obj ManagerAccount.DeleteRsp
function TDeleteRsp:init_from_obj(obj) end
...
function TDeleteRsp:unpack(_) end
...
local function TDeleteReq_from_obj(obj) return setmetatable(obj, TDeleteReq) end

function TDeleteReq.new() return TDeleteReq_from_obj({}) end
---@param obj ManagerAccount.DeleteReq
function TDeleteReq:init_from_obj(obj) end
...
function TDeleteReq:unpack(_) end

ManagerAccount.DeleteReq = TDeleteReq

ManagerAccount.interface = mdb.register_interface(
        ...
        ImportSSHPublicKey = {
            'a{ss}ss', 'u', TImportSSHPublicKeyReq, TImportSSHPublicKeyRsp
        },
        DeleteSSHPublicKey = {
            'a{ss}', '', TDeleteSSHPublicKeyReq, TDeleteSSHPublicKeyRsp
        },
