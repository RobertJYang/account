...
function THttpsTransferCertVerification.new(HttpsTransferCertVerification)
    return THttpsTransferCertVerification_from_obj({
        HttpsTransferCertVerification = HttpsTransferCertVerification
    })
end
...
THttpsTransferCertVerification.from_obj =
    THttpsTransferCertVerification_from_obj

THttpsTransferCertVerification.proto_property = {
    'HttpsTransferCertVerification'
}

THttpsTransferCertVerification.default = {false}

THttpsTransferCertVerification.struct = {
    {name = 'HttpsTransferCertVerification', is_array = false, struct = nil}
}

function THttpsTransferCertVerification:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'HttpsTransferCertVerification',
                      self.HttpsTransferCertVerification, "bool", false, errs,
                      need_convert)
...
local function TUmountRsp_from_obj(obj) return setmetatable(obj, TUmountRsp) end

function TUmountRsp.new(Result) return TUmountRsp_from_obj({Result = Result}) end
---@param obj FileTransfer.UmountRsp
function TUmountRsp:init_from_obj(obj) self.Result = obj.Result end
...
function TUmountRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TUmountRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TUmountRsp:unpack(_) return self.Result end
...
local function TUmountReq_from_obj(obj) return setmetatable(obj, TUmountReq) end

function TUmountReq.new(MountPoint)
    return TUmountReq_from_obj({MountPoint = MountPoint})
end
---@param obj FileTransfer.UmountReq
function TUmountReq:init_from_obj(obj) self.MountPoint = obj.MountPoint end
...
function TUmountReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'MountPoint', self.MountPoint, "string", false,
                      errs, need_convert)

    TUmountReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TUmountReq.proto_property, errs,
                                 need_convert)
    return self
end

function TUmountReq:unpack(_) return self.MountPoint end
...
local function TMountRsp_from_obj(obj) return setmetatable(obj, TMountRsp) end

function TMountRsp.new(Result) return TMountRsp_from_obj({Result = Result}) end
---@param obj FileTransfer.MountRsp
function TMountRsp:init_from_obj(obj) self.Result = obj.Result end
...
function TMountRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TMountRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TMountRsp:unpack(_) return self.Result end
...
local function TMountReq_from_obj(obj) return setmetatable(obj, TMountReq) end
...
TMountReq.struct = {
    {name = 'Src', is_array = false, struct = nil},
    {name = 'MountPoint', is_array = false, struct = nil}
}

function TMountReq:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'Src', self.Src, "string", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'MountPoint', self.MountPoint, "string", false,
                      errs, need_convert)

    TMountReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMountReq.proto_property, errs,
                                 need_convert)
    return self
end

function TMountReq:unpack(_) return self.Src, self.MountPoint end
...
function TStartTransferRsp:init_from_obj(obj) self.TaskId = obj.TaskId end
...
function TStartTransferRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TStartTransferRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TStartTransferRsp:unpack(_) return self.TaskId end
...
function TStartTransferReq.new(SrcUrl, TargetUrl, MaxLength, Uid, Gid,
                               Permission)
...
TStartTransferReq.proto_property = {
    'SrcUrl', 'TargetUrl', 'MaxLength', 'Uid', 'Gid', 'Permission'
}

TStartTransferReq.default = {'', '', 0, 0, 0, 0}

TStartTransferReq.struct = {
    {name = 'SrcUrl', is_array = false, struct = nil},
    {name = 'TargetUrl', is_array = false, struct = nil},
    {name = 'MaxLength', is_array = false, struct = nil},
...
function TStartTransferReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'SrcUrl', self.SrcUrl, "string", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'TargetUrl', self.TargetUrl, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'MaxLength', self.MaxLength, "uint32", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'Uid', self.Uid, "uint32", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'Gid', self.Gid, "uint32", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'Permission', self.Permission, "uint32", false,
                      errs, need_convert)

    TStartTransferReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TStartTransferReq.proto_property, errs,
                                 need_convert)
    return self
end

function TStartTransferReq:unpack(_)
    return self.SrcUrl, self.TargetUrl, self.MaxLength, self.Uid, self.Gid,
           self.Permission
end

FileTransfer.StartTransferReq = TStartTransferReq

FileTransfer.interface = mdb.register_interface(
                             'bmc.kepler.Managers.FileTransfer', {
        HttpsTransferCertVerification = {'b', nil, false, nil, false}
    }, {
        StartTransfer = {
            'a{ss}ssuuuu', 'u', TStartTransferReq, TStartTransferRsp
        },
