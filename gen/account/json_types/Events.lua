...
function TMaskedEventInfo.new(EventCode, EventName)
    return TMaskedEventInfo_from_obj({
        EventCode = EventCode,
        EventName = EventName
    })
end
...
TMaskedEventInfo.struct = {
    ...
    {name = 'EventName', is_array = false, struct = nil}
}

function TMaskedEventInfo:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'EventCode', self.EventCode, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'EventName', self.EventName, "string", false,
                      errs, need_convert)

    TMaskedEventInfo:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMaskedEventInfo.proto_property, errs,
                                 need_convert)
    return self
end

function TMaskedEventInfo:unpack(_) return self.EventCode, self.EventName end
...
function TKeyValueTable:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TKeyValueTable.proto_property, errs,
                                 need_convert)
    return self
end

function TKeyValueTable:unpack(_) return self.Key, self.Value end
...
local function TEventInfo_from_obj(obj)
    obj.MappingTable = utils.from_obj(Events.KeyValueTable, obj.MappingTable,
                                      true)
...
function TEventInfo:init_from_obj(obj) self.MappingTable = obj.MappingTable end
...
TEventInfo.struct = {
    {
        name = 'MappingTable',
        is_array = true,
        struct = Events.KeyValueTable.struct
    }
}
...
function TEventInfo:validate(prefix, errs, need_convert)
    ...
    for _, v in pairs(self.MappingTable) do
        Events.KeyValueTable.new(v.Key, v.Value):validate(prefix, errs,
                                                          need_convert)
    end

    TEventInfo:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TEventInfo.proto_property, errs,
                                 need_convert)
    return self
end

function TEventInfo:unpack(raw) return
    utils.unpack(raw, self.MappingTable, true) end
...
local function TEventMode_from_obj(obj) return setmetatable(obj, TEventMode) end

function TEventMode.new(EventMode)
    return TEventMode_from_obj({EventMode = EventMode})
end
---@param obj Events.EventMode
function TEventMode:init_from_obj(obj) self.EventMode = obj.EventMode end
...
function TEventMode:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'EventMode', self.EventMode, "uint8", false,
                      errs, need_convert)

    TEventMode:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TEventMode.proto_property, errs,
                                 need_convert)
    return self
end

function TEventMode:unpack(_) return self.EventMode end
...
local function TAuxVersion_from_obj(obj) return setmetatable(obj, TAuxVersion) end
...
function TAuxVersion:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'AuxVersion', self.AuxVersion, "string", true,
                      errs, need_convert)

    TAuxVersion:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TAuxVersion.proto_property, errs,
                                 need_convert)
    return self
end

function TAuxVersion:unpack(_) return self.AuxVersion end
...
function TMinorVersion:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'MinorVersion', self.MinorVersion, "string",
                      true, errs, need_convert)

    TMinorVersion:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMinorVersion.proto_property, errs,
                                 need_convert)
    return self
end

function TMinorVersion:unpack(_) return self.MinorVersion end
...
function TMajorVersion:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'MajorVersion', self.MajorVersion, "string",
                      true, errs, need_convert)

    TMajorVersion:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMajorVersion.proto_property, errs,
                                 need_convert)
    return self
end

function TMajorVersion:unpack(_) return self.MajorVersion end
...
local function TDumpRecord_from_obj(obj) return setmetatable(obj, TDumpRecord) end

function TDumpRecord.new(DumpRecord)
    return TDumpRecord_from_obj({DumpRecord = DumpRecord or 0})
end
---@param obj Events.DumpRecord
function TDumpRecord:init_from_obj(obj) self.DumpRecord = obj.DumpRecord or 0 end
...
function TDumpRecord:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'DumpRecord', self.DumpRecord, "uint64", true,
                      errs, need_convert)

    TDumpRecord:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TDumpRecord.proto_property, errs,
                                 need_convert)
    return self
end

function TDumpRecord:unpack(_) return self.DumpRecord end
...
function TMaxEventCount:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'MaxEventCount', self.MaxEventCount, "uint32",
                      true, errs, need_convert)

    TMaxEventCount:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMaxEventCount.proto_property, errs,
                                 need_convert)
    return self
end

function TMaxEventCount:unpack(_) return self.MaxEventCount end
...
TCurEventCount.struct = {
    {name = 'CurEventCount', is_array = false, struct = nil}
}

function TCurEventCount:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'CurEventCount', self.CurEventCount, "uint32",
                      true, errs, need_convert)

    TCurEventCount:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TCurEventCount.proto_property, errs,
                                 need_convert)
    return self
end

function TCurEventCount:unpack(_) return self.CurEventCount end
...
function TVersion:init_from_obj(obj) self.Version = obj.Version or [=[`1.0.0`]=] end
...
local function TMinorCount_from_obj(obj) return setmetatable(obj, TMinorCount) end

function TMinorCount.new(MinorCount)
    return TMinorCount_from_obj({MinorCount = MinorCount or 0})
end
---@param obj Events.MinorCount
function TMinorCount:init_from_obj(obj) self.MinorCount = obj.MinorCount or 0 end
...
function TMinorCount:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'MinorCount', self.MinorCount, "uint32", true,
                      errs, need_convert)

    TMinorCount:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMinorCount.proto_property, errs,
                                 need_convert)
    return self
end

function TMinorCount:unpack(_) return self.MinorCount end
...
local function TMajorCount_from_obj(obj) return setmetatable(obj, TMajorCount) end

function TMajorCount.new(MajorCount)
    return TMajorCount_from_obj({MajorCount = MajorCount or 0})
end
---@param obj Events.MajorCount
function TMajorCount:init_from_obj(obj) self.MajorCount = obj.MajorCount or 0 end
...
function TMajorCount:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'MajorCount', self.MajorCount, "uint32", true,
                      errs, need_convert)

    TMajorCount:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMajorCount.proto_property, errs,
                                 need_convert)
    return self
end

function TMajorCount:unpack(_) return self.MajorCount end
...
TCriticalCount.struct = {
    {name = 'CriticalCount', is_array = false, struct = nil}
}

function TCriticalCount:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'CriticalCount', self.CriticalCount, "uint32",
                      true, errs, need_convert)

    TCriticalCount:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TCriticalCount.proto_property, errs,
                                 need_convert)
    return self
end

function TCriticalCount:unpack(_) return self.CriticalCount end
...
TEventRecordSeq.struct = {
    {name = 'EventRecordSeq', is_array = false, struct = nil}
}

function TEventRecordSeq:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'EventRecordSeq', self.EventRecordSeq, "uint64",
                      true, errs, need_convert)

    TEventRecordSeq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TEventRecordSeq.proto_property, errs,
                                 need_convert)
    return self
end

function TEventRecordSeq:unpack(_) return self.EventRecordSeq end
...
local function THealth_from_obj(obj) return setmetatable(obj, THealth) end

function THealth.new(Health)
    return THealth_from_obj({Health = Health or [=[Normal]=]})
end
---@param obj Events.Health
function THealth:init_from_obj(obj) self.Health = obj.Health or [=[Normal]=] end
...
function THealth:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Health', self.Health, "string", true, errs,
                      need_convert)

    THealth:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, THealth.proto_property, errs,
                                 need_convert)
    return self
end

function THealth:unpack(_) return self.Health end
...
local function TAddSelRsp_from_obj(obj) return setmetatable(obj, TAddSelRsp) end

function TAddSelRsp.new() return TAddSelRsp_from_obj({}) end
---@param obj Events.AddSelRsp
function TAddSelRsp:init_from_obj(obj) end
...
function TAddSelRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TAddSelRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TAddSelRsp:unpack(_) end
...
function TAddSelReq:init_from_obj(obj) self.EventInfo = obj.EventInfo end
...
TAddSelReq.struct = {
    {name = 'EventInfo', is_array = true, struct = Events.KeyValueTable.struct}
}
...
function TAddSelReq:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TAddSelReq.proto_property, errs,
                                 need_convert)
    return self
end

function TAddSelReq:unpack(raw) return utils.unpack(raw, self.EventInfo, true) end
...
function TSetEventActionRsp.new() return TSetEventActionRsp_from_obj({}) end
---@param obj Events.SetEventActionRsp
function TSetEventActionRsp:init_from_obj(obj) end
...
function TSetEventActionRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TSetEventActionRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TSetEventActionRsp:unpack(_) end
...
function TSetEventActionReq.new(EventKeyId, Action)
    return TSetEventActionReq_from_obj({
...
TSetEventActionReq.struct = {
    {name = 'EventKeyId', is_array = false, struct = nil},
    {name = 'Action', is_array = false, struct = nil}
}

function TSetEventActionReq:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.Optional(prefix .. 'EventKeyId', self.EventKeyId, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'Action', self.Action, "uint8", false, errs,
                      need_convert)

    TSetEventActionReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TSetEventActionReq.proto_property, errs,
                                 need_convert)
    return self
end

function TSetEventActionReq:unpack(_) return self.EventKeyId, self.Action end
...
function TSetEventSeverityRsp.new() return TSetEventSeverityRsp_from_obj({}) end
---@param obj Events.SetEventSeverityRsp
function TSetEventSeverityRsp:init_from_obj(obj) end
...
function TSetEventSeverityRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TSetEventSeverityRsp.proto_property,
                                 errs, need_convert)
    return self
end

function TSetEventSeverityRsp:unpack(_) end
...
function TSetEventSeverityReq.new(EventCode, Severity)
    return TSetEventSeverityReq_from_obj({
        EventCode = EventCode,
        Severity = Severity
    })
end
...
function TSetEventSeverityReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'EventCode', self.EventCode, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'Severity', self.Severity, "uint8", false, errs,
                      need_convert)

    TSetEventSeverityReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TSetEventSeverityReq.proto_property,
                                 errs, need_convert)
    return self
end

function TSetEventSeverityReq:unpack(_) return self.EventCode, self.Severity end
...
function TSetAlarmNameRsp.new() return TSetAlarmNameRsp_from_obj({}) end
---@param obj Events.SetAlarmNameRsp
function TSetAlarmNameRsp:init_from_obj(obj) end
...
function TSetAlarmNameRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TSetAlarmNameRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TSetAlarmNameRsp:unpack(_) end
...
function TSetAlarmNameReq:init_from_obj(obj) self.AlarmName = obj.AlarmName end
...
function TSetAlarmNameReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'AlarmName', self.AlarmName, "string", false,
                      errs, need_convert)

    TSetAlarmNameReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TSetAlarmNameReq.proto_property, errs,
                                 need_convert)
    return self
end

function TSetAlarmNameReq:unpack(_) return self.AlarmName end
...
function TCheckEventNameRsp:init_from_obj(obj) self.Result = obj.Result end
...
function TCheckEventNameRsp:validate(prefix, errs, need_convert)
    ...
    validate.OptionalArray(prefix .. 'Result', self.Result, "bool", false, errs,
                           need_convert)

    TCheckEventNameRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TCheckEventNameRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TCheckEventNameRsp:unpack(_) return self.Result end
...
function TCheckEventNameReq:init_from_obj(obj) self.EventName = obj.EventName end
...
TCheckEventNameReq.struct = {
    {name = 'EventName', is_array = true, struct = nil}
}

function TCheckEventNameReq:validate(prefix, errs, need_convert)
    prefix = prefix or ''

    validate.OptionalArray(prefix .. 'EventName', self.EventName, "string",
                           false, errs, need_convert)

    TCheckEventNameReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TCheckEventNameReq.proto_property, errs,
                                 need_convert)
    return self
end

function TCheckEventNameReq:unpack(_) return self.EventName end
...
function TExportEventRsp:init_from_obj(obj) self.TaskId = obj.TaskId end
...
function TExportEventRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TExportEventRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TExportEventRsp:unpack(_) return self.TaskId end
...
function TExportEventReq.new(Path) return
    TExportEventReq_from_obj({Path = Path}) end
---@param obj Events.ExportEventReq
function TExportEventReq:init_from_obj(obj) self.Path = obj.Path end
...
function TExportEventReq:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TExportEventReq.proto_property, errs,
                                 need_convert)
    return self
end

function TExportEventReq:unpack(_) return self.Path end
...
local function TAddEventRsp_from_obj(obj) return setmetatable(obj, TAddEventRsp) end

function TAddEventRsp.new(RecordId)
    return TAddEventRsp_from_obj({RecordId = RecordId})
end
---@param obj Events.AddEventRsp
function TAddEventRsp:init_from_obj(obj) self.RecordId = obj.RecordId end
...
function TAddEventRsp:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'RecordId', self.RecordId, "string", false,
                      errs, need_convert)

    TAddEventRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TAddEventRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TAddEventRsp:unpack(_) return self.RecordId end
...
function TAddEventReq:init_from_obj(obj) self.EventInfo = obj.EventInfo end
...
function TAddEventReq:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TAddEventReq.proto_property, errs,
                                 need_convert)
    return self
end

function TAddEventReq:unpack(raw) return utils.unpack(raw, self.EventInfo, true) end
...
local function TGetMaskedEventListRsp_from_obj(obj)
    obj.MaskedEventList = utils.from_obj(Events.MaskedEventInfo,
                                         obj.MaskedEventList, true)
...
TGetMaskedEventListRsp.struct = {
    {
        name = 'MaskedEventList',
        is_array = true,
        struct = Events.MaskedEventInfo.struct
    }
}
...
function TGetMaskedEventListRsp:validate(prefix, errs, need_convert)
    ...
    for _, v in pairs(self.MaskedEventList) do
        Events.MaskedEventInfo.new(v.EventCode, v.EventName):validate(prefix,
                                                                      errs,
                                                                      need_convert)
    end

    TGetMaskedEventListRsp:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TGetMaskedEventListRsp.proto_property,
                                 errs, need_convert)
    return self
end
...
function TGetMaskedEventListReq.new() return TGetMaskedEventListReq_from_obj({}) end
---@param obj Events.GetMaskedEventListReq
function TGetMaskedEventListReq:init_from_obj(obj) end
...
function TGetMaskedEventListReq:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TGetMaskedEventListReq.proto_property,
                                 errs, need_convert)
    return self
end

function TGetMaskedEventListReq:unpack(_) end
...
function TMaskEventRsp.new() return TMaskEventRsp_from_obj({}) end
---@param obj Events.MaskEventRsp
function TMaskEventRsp:init_from_obj(obj) end
...
function TMaskEventRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TMaskEventRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TMaskEventRsp:unpack(_) end
...
TMaskEventReq.struct = {
    {name = 'EventCode', is_array = false, struct = nil},
    {name = 'MaskState', is_array = false, struct = nil},
...
function TMaskEventReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'EventCode', self.EventCode, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'MaskState', self.MaskState, "uint8", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'Mode', self.Mode, "uint8", false, errs,
                      need_convert)

    TMaskEventReq:remove_error_props(errs, self)
    validate.CheckUnknowProperty(self, TMaskEventReq.proto_property, errs,
                                 need_convert)
    return self
end

function TMaskEventReq:unpack(_) return self.EventCode, self.MaskState,
                                        self.Mode end
...
function TGetEventInfoRsp:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Version', self.Version, "string", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'CurEventCount', self.CurEventCount, "uint16",
                      false, errs, need_convert)
    validate.Optional(prefix .. 'MaxEventCount', self.MaxEventCount, "uint16",
                      false, errs, need_convert)
...
function TGetEventInfoReq.new() return TGetEventInfoReq_from_obj({}) end
---@param obj Events.GetEventInfoReq
function TGetEventInfoReq:init_from_obj(obj) end
...
function TGetEventInfoReq:unpack(_) end
...
function TClearEventListRsp.new() return TClearEventListRsp_from_obj({}) end
---@param obj Events.ClearEventListRsp
function TClearEventListRsp:init_from_obj(obj) end
...
function TClearEventListRsp:unpack(_) end
...
function TClearEventListReq.new() return TClearEventListReq_from_obj({}) end
---@param obj Events.ClearEventListReq
function TClearEventListReq:init_from_obj(obj) end
...
function TClearEventListReq:unpack(_) end
...
function TGetEventListRsp:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'Total', self.Total, "uint16", false, errs,
                      need_convert)
...
local function TGetEventListReq_from_obj(obj)
    obj.QueryParameters = utils.from_obj(Events.KeyValueTable,
                                         obj.QueryParameters, true)
    return setmetatable(obj, TGetEventListReq)
end

function TGetEventListReq.new(StartId, Count, QueryParameters)
    return TGetEventListReq_from_obj({
        StartId = StartId,
        Count = Count,
...
function TGetEventListReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'StartId', self.StartId, "uint16", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'Count', self.Count, "uint16", false, errs,
                      need_convert)
...
function TGetEventListReq:unpack(raw)
    return self.StartId, self.Count,
           utils.unpack(raw, self.QueryParameters, true)
end
...
function TGetAlarmListReq.new(StartId, Count, QueryParameters)
    return TGetAlarmListReq_from_obj({
        StartId = StartId,
        Count = Count,
        QueryParameters = QueryParameters
    })
...
function TMockEventRsp.new() return TMockEventRsp_from_obj({}) end
---@param obj Events.MockEventRsp
function TMockEventRsp:init_from_obj(obj) end
...
function TMockEventRsp:validate(prefix, errs, need_convert)
    ...
    validate.CheckUnknowProperty(self, TMockEventRsp.proto_property, errs,
                                 need_convert)
    return self
end

function TMockEventRsp:unpack(_) end
...
TMockEventReq.proto_property = {
    'EventCode', 'Enabled', 'ObjectIndex', 'MockState'
}
...
function TMockEventReq:validate(prefix, errs, need_convert)
    ...
    validate.Optional(prefix .. 'EventCode', self.EventCode, "string", false,
                      errs, need_convert)
    validate.Optional(prefix .. 'Enabled', self.Enabled, "uint8", false, errs,
                      need_convert)
    validate.Optional(prefix .. 'ObjectIndex', self.ObjectIndex, "uint16",
                      false, errs, need_convert)
    validate.Optional(prefix .. 'MockState', self.MockState, "uint8", false,
                      errs, need_convert)
...
Events.interface = mdb.register_interface('bmc.kepler.Systems.Events', {
    ...
    GetAlarmList = {
        'a{ss}qqa(ss)', 'qa(a(ss))', TGetAlarmListReq, TGetAlarmListRsp
    },
    GetEventList = {
        'a{ss}qqa(ss)', 'qa(a(ss))', TGetEventListReq, TGetEventListRsp
    },
    ...
    GetMaskedEventList = {
        'a{ss}', 'a(ss)', TGetMaskedEventListReq, TGetMaskedEventListRsp
    },
    ...
    SetEventSeverity = {
        'a{ss}sy', '', TSetEventSeverityReq, TSetEventSeverityRsp
    },
