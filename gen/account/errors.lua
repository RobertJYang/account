...
-- Create: 2025-3-15
-- Description: DO NOT EDIT; Code generated by "errors.lua.mako"
...
function M.authorization_failed()
    local err_data = new_error(AuthorizationFailed.name,
                               AuthorizationFailed.format)
...
local InternalError = {
    name = "kepler.account.InternalError",
    format = [=[Internal Error Met.]=],
    severity = 'error'
}
...
function M.auth_type_not_support()
    local err_data = new_error(AuthTypeNotSupport.name,
                               AuthTypeNotSupport.format)
...
function M.auth_algo_not_support()
    local err_data = new_error(AuthAlgoNotSupport.name,
                               AuthAlgoNotSupport.format)
...
function M.session_type_not_support()
    local err_data = new_error(SessionTypeNotSupport.name,
                               SessionTypeNotSupport.format)
...
local NoValidSession = {
    ...
    format = [=[There is no valid session established with the imp]=] ..
        [=[lementation.]=],
    severity = 'error'
}
...
local SessionKickout = {
    ...
    format = [=[You have been forced to log out by the system admi]=] ..
        [=[nistrator.]=],
    severity = 'error'
}
...
local SessionChanged = {
    ...
    format = [=[Account information has been modified, please log ]=] ..
        [=[in again.]=],
    severity = 'error'
}
...
function M.session_limit_exceeded()
    local err_data = new_error(SessionLimitExceeded.name,
                               SessionLimitExceeded.format)
...
function M.resource_already_exists()
    local err_data = new_error(ResourceAlreadyExists.name,
                               ResourceAlreadyExists.format)
...
local UnSupported = {
    name = "kepler.account.UnSupported",
    format = [=[The operate not supported.]=],
    severity = 'error'
}
...
local ExclusiveMode = {
    ...
    format = [=[The session is already exclusive or already exists]=] ..
        [=[ session cannot be created exclusive.]=],
    severity = 'error'
}
...
local PasswordComplexityCheckFail = {
    ...
    format = [=[The property does not meet the password complexity]=] ..
        [=[ requirements.]=],
    severity = 'error'
}
...
function M.password_complexity_check_fail()
    local err_data = new_error(PasswordComplexityCheckFail.name,
                               PasswordComplexityCheckFail.format)
...
function M.invalid_password_same_with_history()
    local err_data = new_error(InvalidPasswordSameWithHistory.name,
                               InvalidPasswordSameWithHistory.format)
...
local IpmiPasswordEmpty = {
    ...
    format = [=[The password for IPMI is empty, please set passwor]=] ..
        [=[d.]=],
    severity = 'error'
}
...
local PasswordNeedReset = {
    ...
    format = [=[This password is an initial password or has been c]=] ..
        [=[hanged by another administrator. Please reset the ]=] ..
...
function M.invalid_password_length()
    local err_data = new_error(InvalidPasswordLength.name,
                               InvalidPasswordLength.format)
...
local PasswordForbidSetComplexityCheck = {
    ...
    format = [=[It is not allowed to enable or disable password co]=] ..
        [=[mplexity check when password complexity check enha]=] ..
        [=[ncement enabled.]=],
    severity = 'error'
}
...
function M.password_forbid_set_complexity_check()
    local err_data = new_error(PasswordForbidSetComplexityCheck.name,
                               PasswordForbidSetComplexityCheck.format)
...
function M.parameter_invalid(val1)
    local err_data = new_error(ParameterInvalid.name, ParameterInvalid.format,
                               val1)
...
local WeakPWDDictInProcess = {
    ...
    format = [=[When weakpwd dictionary is in process, can't expor]=] ..
        [=[t or import again, or check password.]=],
    severity = 'error'
}
M.WeakPWDDictInProcess = WeakPWDDictInProcess.name
---@return Error
function M.weak_pwd_dict_in_process()
    local err_data = new_error(WeakPWDDictInProcess.name,
                               WeakPWDDictInProcess.format)
...
local PasswordInWeakPWDDict = {
    ...
    format = [=[The property cannot be in the weak password dictio]=] ..
        [=[nary.]=],
    severity = 'error'
}
...
function M.password_in_weak_pwd_dict()
    local err_data = new_error(PasswordInWeakPWDDict.name,
                               PasswordInWeakPWDDict.format)
...
local DuringMinimumPasswordAge = {
    ...
    format = [=[The password cannot be changed because it has not ]=] ..
        [=[passed the minimum password age.]=],
    severity = 'error'
}
M.DuringMinimumPasswordAge = DuringMinimumPasswordAge.name
---@return Error
function M.during_minimum_password_age()
    local err_data = new_error(DuringMinimumPasswordAge.name,
                               DuringMinimumPasswordAge.format)
...
local PropertyMemberQtyExceedLimit = {
    ...
    format = [=[The number of items in the property %s exceeds the]=] ..
        [=[ limit.]=],
    severity = 'error'
}
...
function M.property_member_qty_exceed_limit(val1)
    local err_data = new_error(PropertyMemberQtyExceedLimit.name,
                               PropertyMemberQtyExceedLimit.format, val1)
...
function M.user_name_not_exist(val1)
    local err_data = new_error(UserNameNotExist.name, UserNameNotExist.format,
                               val1)
...
function M.incorrect_public_key_format()
    local err_data = new_error(IncorrectPublicKeyFormat.name,
                               IncorrectPublicKeyFormat.format)
...
function M.host_user_management_diabled()
    local err_data = new_error(HostUserManagementDiabled.name,
                               HostUserManagementDiabled.format)
...
local UserFull = {
    name = "kepler.account.UserFull",
    format = [=[User full, cannot add more user]=],
    severity = 'error'
}
...
function M.community_string_contain_space()
    local err_data = new_error(CommunityStringContainSpace.name,
                               CommunityStringContainSpace.format)
...
function M.invalid_community_string_length()
    local err_data = new_error(InvalidCommunityStringLength.name,
                               InvalidCommunityStringLength.format)
...
function M.unsupport_two_factor_param()
    local err_data = new_error(UnsupportTwoFactorParam.name,
                               UnsupportTwoFactorParam.format)
...
function M.import_invalid_keytab()
    local err_data = new_error(ImportInvalidKeytab.name,
                               ImportInvalidKeytab.format)
